  ...
  function SignaturePad(selector, options) {
    ...
    function drawLine(e, newYOffset) {
      ...
      if (settings.drawBezierCurves === true) {
        strokePoints.push({ lx: newX, ly: newY, mx: previous.x, my: previous.y });
      ...
      if (settings.onDraw && typeof settings.onDraw === 'function') settings.onDraw.apply(self);
    }
    ...
    function stopDrawing(e) {
      if (!!e && !(e.type === 'touchend' || e.type == 'touchcancel')) {
        ...
        if (output.length > 0) {
          if (settings.onDrawEnd && typeof settings.onDrawEnd === 'function') settings.onDrawEnd.apply(self);
      ...
      if (settings.output && output.length > 0) $(settings.output, context).val(JSON.stringify(output));
    }
    ...
    function onFormError(errors, context, settings) {
      ...
      if (errors.drawInvalid) context.prepend(['<p class="', settings.errorClass, '">', settings.errorMessageDraw, '</p>'].join(''));
    }
    ...
    */
    ...
    function drawSignature(paths, context, saveOutput) {
      if (settings.autoscale) {
        ...
        context.translate((canvas.width() / scaleFactor - signatureWidth) / 2, (canvas.height() / scaleFactor - signatureHeight) / 2);
      } else {
    ...
    $.extend(self, {
      ...
      regenerate: function (paths) {
        ...
        if (settings.output && $(settings.output, context).length > 0) $(settings.output, context).val(JSON.stringify(output));
      },
      ...
      getSignatureImage: function () {
        ...
        if (!tmpCanvas.getContext && FlashCanvas) FlashCanvas.initElement(tmpCanvas);
