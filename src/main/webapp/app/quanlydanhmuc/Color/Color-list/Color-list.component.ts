...
export class ColorManagementComponent implements OnInit {
  ...
  onEditClick(id: number) {
    const element = this.colors.find((p) => { return p.id === id });
  ...
  private onSuccess(color: Color[] | null, headers: HttpHeaders): void {
    this.totalItems = color.length;
    this.colors = color;
    this.totalPage = Math.ceil(this.totalItems / this.itemsPerPage);
    this.updateFilters();
  }
