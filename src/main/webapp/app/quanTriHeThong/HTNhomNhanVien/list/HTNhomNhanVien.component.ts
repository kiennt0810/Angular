...
import { User } from '../HTNhomNhanVien.model';
...
declare var $: any;
@Component({
    selector: 'jhi-HTNhomNhanVien',
    templateUrl: './HTNhomNhanVien.component.html',
    styleUrls: ['../../quantrihethong.component.scss', './HTNhomNhanVien.component.scss'],
    standalone: true,
    imports: [MatTableModule, FormsModule, MatCheckboxModule, RouterModule, MatSortModule,
    AlertModuleCheck, CommonModule],
})
export class HTNhomNhanVienComponent implements AfterViewInit  {

    displayedColumns:string[] = ['maNhanVien', 'tenNhanVien', 'select'];
    ...
    constructor(
        private userService: HTNhomNhanVienService,
    ...
    private onSaveSuccess(): void {
        this.isSaving = false;
        this.updateFilters();
        this.alert.success('Phân nhóm thành công'); 
      }
    
      private onSaveError(response: HttpErrorResponse): void {
          this.isSaving = false;
          if (response.status !== 200) {
            this.alert.error('Phân nhóm không thành công');
          }
    }
