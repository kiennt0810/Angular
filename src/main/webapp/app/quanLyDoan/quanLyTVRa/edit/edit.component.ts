...
import { IQuanLyTV, QuanLyTV } from '../quanLyTVRa.model';
...
export class TvEditComponent implements OnInit {
  ...
  editForm = new FormGroup({
    ...
    hC_SoThiThuc: new FormControl(quanLyTVTemp.soThiThuc, {
      nonNullable: true,
      validators: [
        Validators.required,
      ],
    }),
    hC_NgayCapTT: new FormControl(quanLyTVTemp.ngayCapTT, {
      nonNullable: true,
      validators: [
        Validators.required,
      ],
    ...
    listTVFileVM: this.listTVFileVM,
  });
  ...
  ngOnInit(): void {
    ...
    this.quanLyTVService.getCurrentData(id).subscribe((result) => {
      ...
      this.ngayNC = this.thanhVien.drHoSoVM.ngayNC;
      this.ngayXC = this.thanhVien.drHoSoVM.ngayXC;
      this.soLuongTV = this.thanhVien.drHoSoVM.soLuongTV;
      this.tTinDoan = this.thanhVien.drHoSoVM;
      this.listFileHC = result.listHCFileVM;
      this.editForm.reset;
      ...
      this.editForm = new FormGroup({
        ...
        tenHSDoan: new FormControl(this.tTinDoan.tenDoan),
        ...
        ngaySinh: new FormControl(this.thanhVien.hcNgoaiGiaoVM.ngaySinh),
        quocGia: new FormControl(this.thanhVien.maQG),
        ...
        hC_NgayCap: new FormControl(this.thanhVien.hcNgoaiGiaoVM.cmndNgayCap),
        hC_NgayHL: new FormControl(this.thanhVien.hcNgoaiGiaoVM.cmndNgayHL),
        hC_Loai: new FormControl(this.thanhVien.hcNgoaiGiaoVM.loaiHC),
        hC_SoThiThuc: new FormControl(this.thanhVien.soThiThuc, {
          nonNullable: true,
          validators: [
            Validators.required,
          ],
        }),
        hC_NgayCapTT: new FormControl(this.thanhVien.ngayCapTT, {
          nonNullable: true,
          validators: [
            Validators.required,
          ],
        ...
        fileHC: new FormControl(this.thanhVien.fileHC),
        listTVFileVM: this.listTVFileVM,
      })
      this.listTVFileVM.clear();
      for (let index = 0; index < this.thanhVien.listTVFileVM.length; index++) {
        ...
        this.listTVFileVM.push(this.listfileUp)
      }
      if(this.thanhVien.idAnhHC != null) {
        this.idAnh = this.thanhVien.idAnhHC ;
        this.downloadImg(this.thanhVien.idAnhHC );
      }

      this.quanLyTVService.getLsTVEdit(this.thanhVien.maHSDoan).subscribe((resultTV) => {
        ...
        for (let index = 0; index < resultTV.length; index++) {
          let tTinTV = new FormGroup({
            ...
            gioiTinh: new FormControl(resultTV[index].gioiTinh ? "Nam" : "Nữ"),
            ...
            maQG: new FormControl(resultTV[index].quocGia),
  ...
  updateTV(): void {
    ...
    if(this.isSaving == true) {
      ...
      payload.append('SoThiThuc', editForm.hC_SoThiThuc == null ? "" : editForm.hC_SoThiThuc);
      payload.append('NgayCapTT', $('#hC_NgayCapTT').first().val());
      ...
      payload.append('listFile', editForm.fileHC)
      if (editForm.fileHC != null) {
        payload.append('fileHoChieu', editForm.fileHC.name)
      }
      payload.append('strLsFileDelete', this.strLsFileDelete);
      console.log(payload.getAll('listFile'));
      if (editForm.hoTen !== null) {
        this.quanLyTVService.update(payload).subscribe({ next: (response) => {
          this.alert.success('Cập nhật thành công thành viên');
          this.redirectTo('/doanRa/quanLyTVRa');
        }, error: response => {
          this.alert.warn("Cập nhật thành viên không thành công! ");
        } });
      }
    }
  }
